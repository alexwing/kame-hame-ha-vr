...
public class OVRPlayerController : MonoBehaviour
{
    ...
    protected Camera CameraRig = null;
   ...
   // private OVRPose? InitialPose;
    ...
    public bool SkipMouseRotation = false; // It is rare to want to use mouse movement in VR, so ignore the mouse by default.
    ...
    void Awake()
    {
        ...
        Camera[] CameraRigs = gameObject.GetComponentsInChildren<Camera>();
    ...
    */
    ...
    protected virtual void UpdateController()
    {
        ...
        */
        CameraHeight = CameraRig.transform.localPosition.y;
        ...
        moveDirection +=   Camera.main.transform.forward  * SimulationRate * Time.deltaTime;

        // moveDirection += MoveThrottle * SimulationRate * Time.deltaTime;
        if (Input.GetButton("Jump") || Input.GetKey(KeyCode.Space))
        {
            //  Jump();				
            moveDirection.y += JumpForce * 100 * Time.deltaTime;
        }
        // Gravity
      ...
      //  if (EnableLinearMovement)
        {
            ...
            Debug.Log(moveForward + "-" + moveLeft + "-" + moveRight + "-" + moveBack);
            bool dpad_move = false;
            //Debug.Log(Input.GetAxis("Oculus_GearVR_LIndexTrigger"));
           /* if (OVRInput.Get(OVRInput.Button.DpadUp) || Input.GetAxis("Oculus_GearVR_LIndexTrigger") < 0)
            {
                moveForward = true;
                dpad_move = true;
            ...
            if (OVRInput.Get(OVRInput.Button.DpadDown) || Input.GetAxis("Oculus_GearVR_LIndexTrigger") > 0)
            {
                moveBack = true;
                dpad_move = true;
            }
        ...
        if (EnableRotation)
        {
            ...
            */
            ...
            if (!SkipMouseRotation)
            {
                ...
                //for xbox on editor
                // euler.y += Input.GetAxis("Horizontal") * rotateInfluence * 0.20f;
                // euler.x -= Input.GetAxis("Vertical") * rotateInfluence * 0.20f;

              //  euler.y += Input.GetAxis("Oculus_GearVR_LThumbstickX") * rotateInfluence * 0.20f;
             //   euler.x += Input.GetAxis("Oculus_GearVR_LThumbstickY") * rotateInfluence * 0.20f;
            ...
            if (SnapRotation)
            {
                ...
                /*
                if (XboxController.leftTrigger >0){
                    euler.y -= XboxController.leftTrigger * rotateInfluence * 0.20f;
                }

                if (XboxController.rightTrigger > 0)
                {
                    euler.y += XboxController.rightTrigger * rotateInfluence * 0.20f;
                }
                */
            }
            else
